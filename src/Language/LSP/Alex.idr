...
record Premise where
  ...
  multiplicity : String
  isImplicit : Bool
%runElab deriveJSON defaultOpts `{Alex.Premise}
...
record Metavar where
  ...
  location : Maybe Location
  type : String
  premises : List Alex.Premise
%runElab deriveJSON defaultOpts `{Alex.Metavar}
...
||| }
export
alexCmd : Ref Ctxt Defs
           => Ref Syn SyntaxInfo
           => Ref ROpts REPLOpts
           => Ref LSPConf LSPConfiguration
           => Core (List Metavar)
alexCmd = do
  logI Metavars "Fetching metavars for ALEX"
  c <- getColor
  setColor False
  holes <- Alex.getUserHolesData
